...
function getSingleAnswer(id, callback) {
	query("select * from answers where answerid = " + id + ";", callback);
}
...
function insertAnswer(values, callback) {
	...
	query("INSERT INTO answers(userid, questionid, response, gender)" +
...
function deleteAllAnswers(callback) {
	query("truncate table answers", callback);
}
...
function deleteParticipants(callback) {
	query("truncate table participants", callback);
}

//toggle the 'winner' field of a specific participant
function updateWinner(email, callback) {
	query ("update participants set winner = not winner where email = '" + email + "'", callback);
}

//delete all winners
function deleteWinners(callback) {
	query("update participants set winner = 0 where winner = 1", callback);
}

//export participants to CSV-file without the winner field
function exportParticipants(callback) {
	query("SELECT email, name FROM participants", callback);
...
from (select questionid, sum(case when response='a' then 1 else 0 end) a, sum(case when response='b' then 1 else 0 end) b from answers group by questionid) x
*/
}
...
select *, greatest(a,b) as greatest, (a+b) as total, if(a>b, 'a', 'b') as mostFreq, round((a/(a+b)*100),2) as a_, round((b/(a+b)*100),2) as b_ 
from (select questionid, sum(case when response='a' then 1 else 0 end) a, sum(case when response='b' then 1 else 0 end) b 
from answers, participants where answers.userid=participants.userid and bouvet=1 group by questionid) x;
*/
}
...
select *, greatest(a,b) as greatest, (a+b) as total, if(a>b, 'a', 'b') as mostFreq, round((a/(a+b)*100),2) as a_, round((b/(a+b)*100),2) as b_ 
from (select questionid, sum(case when response='a' then 1 else 0 end) a, sum(case when response='b' then 1 else 0 end) b 
from answers where gender='m' group by questionid) x;
*/
}
...
select *, greatest(a,b) as greatest, (a+b) as total, if(a>b, 'a', 'b') as mostFreq, round((a/(a+b)*100),2) as a_, round((b/(a+b)*100),2) as b_ 
from (select questionid, sum(case when response='a' then 1 else 0 end) a, sum(case when response='b' then 1 else 0 end) b 
from answers where gender='f' group by questionid) x;
*/
}
...
/*
select count(*) as total from answers; select count(*) as bouvet from answers,participants 
where answers.userid = participants.userid and bouvet=1; select count(*) as male from answers where gender='m'; 
select count(*) as female from answers where gender='f';
*/
}


function getCurrentAnswers(id, callback) {
	query("Select questionid, response from answers where userid=" + id, callback);
}
...
sum(case when response='a' then 1 else 0 end) a, sum(case when response='b' then 1 else 0 end) b from answers group by questionid) x;
*/
}
